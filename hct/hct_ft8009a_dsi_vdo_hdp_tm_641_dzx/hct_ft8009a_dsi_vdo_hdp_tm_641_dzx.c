...
static unsigned int lcm_compare_id(void)
{
	...
	#if defined(BUILD_LK)
	printf("hct_ft8009a_dsi_vdo_hdp_tm_641_dzx adc = %x adcdata= %x %x, ret=%d, ADC_Min_VALUE=%x\r\n", rawdata, adcdata[0], adcdata[1],ret,ADC_MIN_VALUE);
	#else
	printk("hct_ft8009a_dsi_vdo_hdp_tm_641_dzx adc = %x adcdata= %x %x, ret=%d, ADC_Min_VALUE=%x\r\n", rawdata, adcdata[0], adcdata[1],ret,ADC_MIN_VALUE);
	#endif
...
LCM_DRIVER hct_ft8009a_dsi_vdo_hdp_tm_641_dzx = 
{
	.name			= "hct_ft8009a_dsi_vdo_hdp_tm_641_dzx",
