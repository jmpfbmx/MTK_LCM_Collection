...
static unsigned int lcm_compare_id(void)
{
	...
	#if defined(BUILD_LK)
	printf("hct_ft8009a_dsi_vdo_hdp_tm_641_YKL adc = %x adcdata= %x %x, ret=%d, ADC_Min_VALUE=%x\r\n", rawdata, adcdata[0], adcdata[1],ret,ADC_MIN_VALUE);
	#else
	printk("hct_ft8009a_dsi_vdo_hdp_tm_641_YKL adc = %x adcdata= %x %x, ret=%d, ADC_Min_VALUE=%x\r\n", rawdata, adcdata[0], adcdata[1],ret,ADC_MIN_VALUE);
	#endif
	if((rawdata > ADC_MIN_VALUE) && (rawdata < ADC_MAX_VALUE))
	{ 
	     array[0]=0x00043700;
	     ...
	     #if defined(BUILD_LK)
	     printf("FT8009a_ykl compare-LK:0x%02x,0x%02x,0x%02x\n", id_high, id_low, id);
	     #else
	     printk("FT8009a_ykl compare:0x%02x,0x%02x,0x%02x,\n", id_high, id_low, id);
	     #endif
...
LCM_DRIVER hct_ft8009a_dsi_vdo_hdp_tm_641_ykl = 
{
	.name			= "hct_ft8009a_dsi_vdo_hdp_tm_641_ykl",
